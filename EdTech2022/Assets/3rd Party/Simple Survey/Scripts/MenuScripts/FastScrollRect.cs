...
/// <summary>
/// https://forum.unity3d.com/threads/make-scrolling-move-faster-in-scrollrect.375116/
///
/// Code:
...
[AddComponentMenu("UI/Fast Scroll Rect", 37)]
[SelectionBase]
[ExecuteInEditMode]
[DisallowMultipleComponent]
[RequireComponent(typeof(RectTransform))]
public class FastScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup, IPointerUpHandler {
    public enum MovementType {
        Unrestricted,
        // Unrestricted movement -- can scroll forever
        Elastic,
    ...
    public virtual void OnInitializePotentialDrag(PointerEventData eventData) {
        ...
        //Removed so that not every touch stops the previous scrolling!
        //m_Velocity = Vector2.zero;

        //Next lines are taken from OnBeginDrag. If OnBeginDrag is fired the initial touch position from OnInitializePotentialDrag 
        //can be miles away from the first occurance of OnBeginDrag. And the first and maybe only OnDrag has the same
        //PointerEventData like the OnBeginDrag. If the touch is very fast and only one OnDrag is fired, scrolling doesn't starts.
        //So we prefer to store the real start position of the touch and the drag will start even if the touch was very fast.
        UpdateBounds();
    ...
    public virtual void OnEndDrag(PointerEventData eventData) {
        if (eventData.button != PointerEventData.InputButton.Left)
            return;
        //OnEndDrag can be fired miles away from the last OnDrag position so we finally call OnDrag with the
        //real end position
        OnDrag(eventData);
        m_Dragging = false;
    }
    ...
    //AXS: Reacts on pointer up and starts the scrolling, if OnBeginDrag was not fired, if the touch move was very fast
